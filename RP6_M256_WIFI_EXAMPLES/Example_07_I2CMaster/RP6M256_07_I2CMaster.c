 ...
 * it is called on the RP6 hardware...) which is connected to PINJ3 / INT1_PI12
 * of the Microcontroller. We will NOT use the interrupt
 ...
 * interrupt service routine of an external Interrupt. 
 ...
 * accordingly.
 * 
 * In this example the Status of the Bumpers, ACS and RC5 Remote Control
 * reception is transmitted with the WIFI Interface. 
 *
 * 
 ...
 * Software, there is a Library called "RP6M256_I2CMasterLib" which will
 * be used starting in the next example. This contains several additional
 * functions which will be explained later on. 
 * This Library has an API equivalent to RP6Lib that can be used nearly
 * the same way, with only some small differences. 
 *
 ...
 */
void task_checkINT(void)
{
	if(!block && (PINJ & INT1_PI12))
	{
		block = true; // Block further requests and wait until 
					  // this request has been processed.
		I2CTWI_requestRegisterFromDevice(I2C_RP6_BASE_ADR, INT_STATUS_CHECK, 0, 3);
	}
}
...
{
	if(dataRequestID == INT_STATUS_CHECK) // We only have one request source, so
	{                                      // we leave out the switch-case from the
		...
		{
			writeString_P_WIFI("- ACS state changed L: ");
			...
			writeChar_WIFI('\n');
			setLED1(interrupt_status.obstacleRight);
			setLED4(interrupt_status.obstacleLeft);
		}
		...
		if(compare & 0b00000110)
		{
			...
			writeString_P_WIFI(" - Bumpers changed: ");
		...
		{
			uint8_t readBuf[2];
			writeString_P_WIFI("Received RC5 Transmission: ");
			I2CTWI_transmitByte(I2C_RP6_BASE_ADR,I2C_REG_RC5_ADR);
			I2CTWI_readBytes(I2C_RP6_BASE_ADR, readBuf, 2);
			writeString_P_WIFI("ADR:");writeInteger_WIFI(readBuf[0],DEC);
			writeString_P_WIFI(" | DATA:");writeInteger_WIFI(readBuf[1],DEC);
			writeString_P_WIFI("\n");
		}
