 ...
 * ****************************************************************************
 * Example: SELFTEST PROGRAM - NEW VERSION 1.5 with Display support
 ...
 *
 * ----------------------------------------------------------------------
 * UPDATE v. 1.5 - 25.07.2012 - added CR+LF support for new RobotLoader
 * Terminal settings which were required because of the new WLAN Module.
 * ----------------------------------------------------------------------
 *
 * ----------------------------------------------------------------------
	...
	{				
		...
		if(receiveBuffer[buffer_pos]=='\n' || receiveBuffer[buffer_pos]=='\r') {
			if(last_char)  // Check for \r\n or \n\r sequences --> ignore. 
			{
				...
				if(receiveBuffer[buffer_pos]=='\r' && last_char=='\n') {
					last_char = 0;
					buffer_pos = 0;
					return 0;
				}
			}
			last_char = receiveBuffer[buffer_pos];
...
#ifdef FACTORY
		writeString_P("######### FACTORY VERSION (!! 7.2V !!)  v. 1.5 - 25.07.2012 #########\n"); 
#endif
#ifdef HOME
		writeString_P("######### HOME VERSION                  v. 1.5 - 25.07.2012 #########\n"); 
#endif
	    writeString_P("#####################################################################\n"); 
